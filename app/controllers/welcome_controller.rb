class WelcomeController < ApplicationController
  ...
  def hook
    if user = User.where(hatena_id: params[:username]).first && user.hatena_bookmark_web_hook_key == params[:key] && params[:status] == "add"
      ...
      conn.post '/api/chat.postMessage', { token: user.slack_token, channel: 'G024ZC07C', text: 'kimura api text', username: username}
    end
    render text: 'OK'
  end
end
